...
(function () { 
    ...
    function setCookie(list) { 
        return Promise.all(list.map(item => {
            return window.cookieStore.set({ 
                name: item.name, value: item.token, domain: 'mihoyo.com', sameSite: 'none' 
            }) 
        })) 
    } 
    ...
    function getGachaUrl() {
        let uid = ''
        let token = ''
        fetch(`https://webapi.account.mihoyo.com/Api/login_by_cookie?t=${Date.now()}`, { credentials: 'include' })
            .then(res => res.json())
            .then(res => {
                uid = res.data.account_info.account_id
                token = res.data.account_info.weblogin_token
                return fetch(`https://api-takumi.mihoyo.com/auth/api/getMultiTokenByLoginTicket?login_ticket=${token}&token_types=3&uid=${uid}`, { credentials: 'include' })
            })
            .then(res => res.json())
            .then(res => {
                return window.cookieStore.set({ name: 'stuid', value: uid, domain: 'mihoyo.com' })
                    .then(() => {
                        return setCookie(res.data.list, 0)
                    })
                    .then(() => {
                        return fetch('https://api-takumi.mihoyo.com/binding/api/getUserGameRolesByCookie?game_biz=hk4e_cn', { credentials: 'include' })
                    })
            })
            .then(res => res.json())
            .then(res => {
    ...
    if (document.querySelector('#root').__vue__.$route.path === '/account/home') {
        setTimeout(() => getGachaUrl(), 2000)
    }
})();
